...
- **T-66** `site/src/pages/foundations/layout.astro`: Added visual demos for all four layout concepts — proportional width bars (content / narrow / prose), a semantic-landmark wireframe (header / main+article / aside / footer), a live `auto-fill` card grid that reflows on narrow viewports, a radius hierarchy row showing all five `--radius-*` tokens, and a z-index stacking ladder for all seven `--z-*` tokens. Reference tables moved into accordions.
