...
- **T-60** `tokens/components.css`: Added `--pagination-font-size`, `--quote-footer-font-size`, and `--code-font-size` tokens — previously the font sizes for these elements were hardcoded directly in `component-classes.css`, bypassing the token override API.
