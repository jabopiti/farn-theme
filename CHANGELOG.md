...
- **T-58** `site/src/pages/components/layout.astro`: Section transitions restructured — each of the six patterns (layered overlap, sine wave, convex arc, organic blob, diagonal cut, stacked card reveal) now has its own `<h3>` section with a live demo visible outside the accordion. Fixed diagonal cut demo: `::before` pseudo-element was covered by `data-theme="dark"` setting `background` on the same element; fixed by adding `background: transparent` to `.section-skew` in `component-classes.css` so the component self-corrects. Full technical reference (pattern guide table, code examples, token tables) moved into a single "Full reference" accordion.
