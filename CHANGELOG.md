...
### Added
- **T-48** `tokens/component-classes.css`: `hr`/`.separator` with default, strong, subtle, ghost variants; `.separator-labeled` text-between-rules pattern using internal `--_sep-line` property so strength modifiers work on labeled variant too; documented in Components › Layout
