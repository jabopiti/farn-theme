...
### Added
- **T-28** `tokens/components.css`: `--stack-scale-step` (0.03), `--stack-top` (0px), `--stack-radius` (radius-xl) Tier-3 tokens for the stacked card reveal pattern
- **T-28** `tokens/component-classes.css`: `.stack-container` and `.stack-section` CSS classes — pure sticky-scroll depth effect, no JS; `nth-last-child` defaults assign `--stack-depth`/`--stack-index` for 2–5 section stacks (last section = front card, full-size); `prefers-reduced-motion` removes scale transform; `100dvh` with `100vh` fallback; `--stack-top` token for fixed-nav offset — shipped in `dist/farn-components.css`
- **T-28** `site/src/pages/components/layout.astro`: Stacked card reveal fully documented — self-contained scrollable demo, anatomy, constraints (content within `100dvh`, 3–5 sections, no mixing with other divider patterns), `--stack-top` nav-offset guidance, token reference, and patterns table updated; "Upcoming patterns" stub removed
